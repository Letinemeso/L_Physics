...
namespace LPhys
{
	...
	private:
		Intersection_Data get_precise_time_ratio_of_collision(const LEti::Object_2D& _first, const LEti::Object_2D& _second, float _min_ratio, float _max_ratio) const;
		Intersection_Data collision__moving_vs_moving(const LEti::Object_2D& _moving_1, const LEti::Object_2D& _moving_2) const;
		Intersection_Data collision__moving_vs_static(const LEti::Object_2D& _moving, const LEti::Object_2D& _static) const;

        LEti::Geometry::Simple_Intersection_Data collision__static_vs_point(const LEti::Object_2D& _static, const glm::vec3& _point) const;

	public:
		Intersection_Data objects_collide(const LEti::Object_2D& _first, const LEti::Object_2D& _second) const;
